...
- **短信是半残废**：**发（MO）可用**——MM 的 Messaging 接口实测发送成功（PDU 模式，短信中心 `+8613800745500`）；**收（MT）不可靠**——模块声明 `+CSMS: 0,1,1,1`（MO/MT 全支持），但实测真实投递进不来（`+CPMS: "SM",0,50` 始终为空）。原因很现实：2G 已退网没有短信回落通道、这块 2013 年的固件对 LTE 短信（NAS/SGs）支持不完整、运营商与手机端已全面转向 5G消息（RCS）——发送方以为在发短信，实际走 RCS/网关，根本落不到 SIM 卡存储里。当作"能发、收随缘"即可；
